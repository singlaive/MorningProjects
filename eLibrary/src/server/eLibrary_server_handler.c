...
static SYSTEM_STATUS Map_InsertElement(uint32_t *p_map, uint32_t map_size, uint32_t new_element)
{
	SYSTEM_STATUS result = COMMON_STATUS_OK;
	uint32_t *p_empty_slot = NULL;
	uint32_t i;
	...
	for(i = 0U; i < map_size; i++)
	{
		...
		{
			if (NULL != p_empty_slot)
			{
				p_empty_slot = &p_map[i];
			}
		}
	}
	...
	if (NULL != p_empty_slot)
	{
		*p_empty_slot = new_element;
	}
	else if (i == map_size)
	{
		COMMON_DIAG_FAIL("Map full!");
		result = COMMON_STATUS_FAIL;
	}
	else
	{
		COMMON_DIAG_INFO("Element exists.");
	}

	return result;
}

static SYSTEM_STATUS Map_RemoveElement(uint32_t *p_map, uint32_t map_size, uint32_t shit_element)
{
	SYSTEM_STATUS result = COMMON_STATUS_OK;
	uint32_t i;
	...
	if (i == map_size)
	{
		COMMON_DIAG_WARN("Did not find id %#x", shit_element);
		result = COMMON_STATUS_FAIL;
	}
...
SYSTEM_STATUS eLibrary_SVR_Handler_AddUser(uint32_t conn_handle, void * job_spec, SERVER_RESPONSE *response_data)
{
	SYSTEM_STATUS result;
	USER_JOB_SPEC *user_job = (USER_JOB_SPEC*)job_spec;
	PUBLIC_USER_INFO user_info;
	uint32_t user_id;
	
	result = g_server_context.db_engine.createAccount(user_job->user_name, &user_id);
	result = g_server_context.db_engine.getUserPublicInfo(user_id, &user_info);

	result = response_data->num_objects = 1U;
	memcpy(response_data->object.users, &user_info, sizeof(PUBLIC_USER_INFO));
}

SYSTEM_STATUS eLibrary_SVR_Handler_BrowseBooks(uint32_t conn_handle, void * job_spec, SERVER_RESPONSE *response_data)
{
	SYSTEM_STATUS result;
	BOOK_JOB_SPEC *book_job = (BOOK_JOB_SPEC*)job_spec;
	uint32_t num_books = 0U;
	COMMON_HANDLE book_ids[MAX];
	DB_SEARCH_CRITERIA search_criteria;
	PUBLIC_BOOK_INFO book_info;

	memcpy(&filter_id, payload, sizeof(uint32_t));
	memcpy(&sort_id, payload+sizeof(uint32_t), sizeoaf(uint32_t));

	result = g_server_context.db_engine.findBooksByFilter(book_job->user_id, book_job->filter_bitmap, &num_books, book_ids);
	result = g_server_context.db_engine.sortBooks(book_ids, num_books);

	response_data->num_objects = num_books;
	for (i = 0U; i < num_books; i++)
	{
		result = g_server_context.db_engine.getBookPublicInfo(book_ids[i], &book_info);
		memcpy(response_data.->bject.books+i*sizeof(PUBLIC_BOOK_INFO), &book_info, sizeof(PUBLIC_BOOK_INFO));
	}
}

SYSTEM_STATUS eLibrary_SVR_Handler_BrowseUsers(uint32_t conn_handle, void * job_spec, SERVER_RESPONSE *response_data)
{
	SYSTEM_STATUS result;
	USER_JOB_SPEC *user_job = (USER_JOB_SPEC*)job_spec;
	uint32_t num_users = 0U;
	COMMON_HANDLE user_ids[MAX];
	PUBLIC_BOOK_INFO user_info;

	result = g_server_context.db_engine.findUsersByFilter(user_job->user_id, user_job->filter_id, &num_users, user_ids);
	result = g_server_context.db_engine.sortUsers(user_ids, num_users);

	response_data.num_objects = num_users;
	for (i = 0U; i < num_users; i++)
	{
		result = g_server_context.db_engine.getUserPublicInfo(user_ids[i], &user_info);
		memcpy(response_data.object.users+i*sizeof(PUBLIC_USER_INFO), &user_info, sizeof(PUBLIC_USER_INFO));
	}
}

SYSTEM_STATUS eLibrary_SVR_Handler_AddFriend(uint32_t conn_handle, void * job_spec, SERVER_RESPONSE *response_data)
{
	SYSTEM_STATUS result = COMMON_STATUS_OK;
	...
	result = g_server_cxt.db.findUsersByFilter(job_spec->user_id, job_spec->filter_id, &num_users, user_ids);
	COMMON_HDL_getObject(job_spec->user_handle, &user_data);
	for (i = 0U; i < num_users; i++)
	{
		result = Map_InsertElement(user_data->friendsMap, user_ids[i]);
		if (COMMON_STATUS OK != result)
		{
			COMMON_DIAG_WARN("Map full (size %d)", MAX_NO_FRIENDS);
			break;
		}
		else
		{
			user_data->numFriends++;
		}
	}

	if (COMMON_STATUS_OK == result)
	{
		response_data->respond.response_status = SERVER_RESPONSE_STATUS_OK;
	}
	else
	{
		response_data->respond.response_status = SERVER_RESPONSE_STATUS_FAIL;
	}

	return result;
}

SYSTEM_STATUS eLibrary_SVR_Handler_RmFriend(uint32_t conn_handle, void * job_spec, SERVER_RESPONSE *response_data)
{
	SYSTEM_STATUS result = COMMON_STATUS_OK;
	USER_INFO *user_data = NULL;
	uint32_t num_users = 0U;
	...
	for (i = 0U; i < num_users; i++)
	{
		result = Map_RemoveElement(user_data->friendsMap, user_ids[i]);
		if (COMMON_STATUS_OK == result)
		{
			user_data->numFriends--;
		}
...
SYSTEM_STATUS eLibrary_SVR_Handler_AddBookDescription(uint32_t conn_handle, void * job_spec, SERVER_RESPONSE *response_data)
{
	SYSTEM_STATUS result = COMMON_STATUS_OK;
	BOOK_INFO *book_data = NULL;

	COMMON_HDL_getObject(job_spec->book_handle, &book_data);
	strncpy(book_data->desp, spec->book_desp, strlen(spec->desp)+1U);
